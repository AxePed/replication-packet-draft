...
table(data$simplicity, data$any_award)
chisq.test(data$simplicity, data$any_award)
#Figure 1box plot ANOVA
ggplot(data, aes(x = as.factor(simplicity), y = intended_reaction)) +
  geom_boxplot() +
  labs(title = "Box Plot of Song Simplicity and Intended Reaction",
       x = "Simplicity",
       y = "Intended Reaction") +
  theme_minimal()
#ANOVA
ANOVA <- aov(intended_reaction~simplicity,data=data)
summary(ANOVA)
#figure 2SCatterplot
linear_plot <- plot(data$intended_reaction, data$instruments)
print(linear_plot)

# add x line and y line for means
meany <- mean(data$instruments)
meanx <- mean(data$intended_reaction)

abline(v = meanx, col = "black")
abline(h = meany, col = "black")

linear_relationship <- lm(data$instruments ~ intended_reaction, data = data)
